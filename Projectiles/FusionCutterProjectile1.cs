...
namespace BonusVanillaStuff.Projectiles
{
	...
	//~~~~~~~~~~~~~NOTE: THE MAJORITY OF THE BELOW CODE IS DIRECTLY FROM THE EXAMPLE LASER WEAPON FROM EXAMPLEMOD~~~~~~~~~~~~
	...
	public class FusionCutterProjectile1 : ModProjectile
	{
		...
		public void DrawLaser(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 unit, float step, int damage, float rotation = 0f, float scale = 1f, float maxDist = 2000f, Color color = default(Color), int transDist = 50)
		{
			...
			for (float i = transDist; i <= 300; i += step) //upper limit was: i <= Distance;          //CHANGING i TO A FLAT NUMBER WORKS, SETTING DISTANCE = FLAT NUMBER DOESN'T
			{
				Color c = Color.White;
				var origin = start + i * unit;
				spriteBatch.Draw(texture, origin - Main.screenPosition,
					new Rectangle(0, 26, 28, 26), i < transDist ? Color.Transparent : c, r,    //new Rectangle(0, 26, 28, 26), i < transDist ? Color.Transparent : c, r, 
					new Vector2(26 * .5f, 26 * .5f), scale, 0, 0);                             //new Vector2(28 * .5f, 26 * .5f), scale, 0, 0);             
			}
			...
			spriteBatch.Draw(texture, start + (300 + step) * unit - Main.screenPosition,
				new Rectangle(0, 52, 35, 26), Color.White, r, new Vector2(28 * .5f, 26 * .5f), scale, 0, 0); // Rectangle(xpos, ypos, width, height);
		}
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
		{
			...
			return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), player.Center,
				player.Center + unit * 322, 22, ref point); //322 was: Distance
		}

		// Set custom immunity time on hitting an NPC
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			target.immune[Projectile.owner] = 5;//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~THIS LINE IS WHAT DETERMINES HOW OFTEN THE DAMAGE TICKS~~~~~~~~~~~~~~~~~~~~~~~
		}
		...
		/*
		 * Sets the end of the laser position based on where it collides with something ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~IMPORTANT! CHECK LATER
		 */
